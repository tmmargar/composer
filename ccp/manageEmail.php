...
if (Constant::MODE_EMAIL == $mode) {
    ...
    foreach ($to as $toEach) {
        ...
        $email = new Email(debug: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_DEBUG), fromName: array(Constant::NAME_STAFF), fromEmail: array(Constant::EMAIL_STAFF()), toName: array($toArray[0]), toEmail: array($toArray[1]), ccName: NULL, ccEmail: NULL, bccName: NULL, bccEmail: NULL, subject: $subject, body: $body);
        $output .= "  aryMessages.push(\"" . $email->sendEmail() . "\");\n";
    }
    $output .= "  if (aryMessages.length > 0) {display.showMessages({messages: aryMessages});}\n</script>\n";
}
$resultList = $entityManager->getRepository(entityName: Constant::ENTITY_PLAYERS)->getActives();
...
$buttonEmail = new FormControl(debug: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_DEBUG), accessKey: Constant::ACCESSKEY_EMAIL, autoComplete: NULL, autoFocus: false, checked: NULL, class: NULL, cols: NULL, disabled: false, id: EMAIL_FIELD_NAME . "_2", maxLength: NULL, name: EMAIL_FIELD_NAME . "_2", onClick: NULL, placeholder:NULL, readOnly: false, required: NULL, rows: NULL, size: NULL, suffix: NULL, type: FormControl::TYPE_INPUT_SUBMIT, value: ucwords(EMAIL_FIELD_NAME), wrap: NULL, noValidate: false);
...
$selectTo = new FormSelect(debug: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_DEBUG), accessKey: Constant::ACCESSKEY_TO, class: array("tom-select"), disabled: false, id: TO_FIELD_NAME, multiple: true, name: TO_FIELD_NAME . "[]", onClick: NULL, readOnly: false, size: 5, suffix: NULL, value: NULL);
$output .= $selectTo->getHtml();
foreach ($resultList as $player) {
    $option = new FormOption(debug: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_DEBUG), class: NULL, disabled: false, id: NULL, name: NULL, selectedValue: NULL, suffix: NULL, text: $player->getPlayerName(), value: $player->getPlayerName() . ":" . $player->getPlayerEmail());
    $output .= $option->getHtml();
}
$output .= "  </select>\n";
$output .= " </div>\n";
$output .= " <div class=\"responsive-cell responsive-cell-label responsive-cell--head\"><label for=\"" . SUBJECT_FIELD_NAME . "\">Subject:</div>\n";
$textBoxEmail = new FormControl(debug: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_DEBUG), accessKey: Constant::ACCESSKEY_SUBJECT, autoComplete: NULL, autoFocus: false, checked: NULL, class: NULL, cols: NULL, disabled: false, id: SUBJECT_FIELD_NAME, maxLength: 100, name: SUBJECT_FIELD_NAME, onClick: NULL, placeholder:NULL, readOnly: false, required: true, rows: NULL, size: 41, suffix: NULL, type: FormControl::TYPE_INPUT_TEXTBOX, value: NULL, wrap: NULL, noValidate: false);
...
$textAreaBody = new FormControl(debug: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_DEBUG), accessKey: Constant::ACCESSKEY_BODY, autoComplete: NULL, autoFocus: false, checked: NULL, class: NULL, cols: 43, disabled: false, id: BODY_FIELD_NAME, maxLength: NULL, name: BODY_FIELD_NAME, onClick: NULL, placeholder:NULL, readOnly: false, required: false, rows: 10, size: NULL, suffix: NULL, type: FormControl::TYPE_INPUT_TEXTAREA, value: NULL, wrap: "hard", noValidate: false);
