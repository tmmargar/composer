...
if (Constant::MODE_CREATE == $mode || Constant::MODE_MODIFY == $mode) {
    ...
    $inventories = $entityManager->getRepository(Constant::ENTITY_INVENTORIES)->getById(inventoryId: (int) $ids);
    ...
    if (Constant::MODE_CREATE == $mode || (Constant::MODE_MODIFY == $mode && $ids != DEFAULT_VALUE_BLANK)) {
        ...
        foreach ($ary as $id) {
            $inventoryTypes = $entityManager->getRepository(Constant::ENTITY_INVENTORY_TYPES)->getNotConfigured(inventoryTypeId: Constant::MODE_MODIFY == $mode ? (int) $id : NULL);
            if (NULL !== $inventoryTypes) {
                ...
                $selectInventoryType = new FormSelect(debug: SessionUtility::getValue(SessionUtility::OBJECT_NAME_DEBUG), accessKey: Constant::ACCESSKEY_LIMIT_TYPE, class: NULL, disabled: false, id: INVENTORY_TYPE_ID_FIELD_NAME . "_" . $id, multiple: false, name: INVENTORY_TYPE_ID_FIELD_NAME . "_" . $id, onClick: NULL, readOnly: false, size: 1, suffix: NULL, value: NULL);
                $output .= " <div class=\"responsive-cell responsive-cell-value\">" . $selectInventoryType->getHtml();
                $option = new FormOption(debug: SessionUtility::getValue(SessionUtility::OBJECT_NAME_DEBUG), class: NULL, disabled: false, id: NULL, name: NULL, selectedValue: count($inventories) > 0 ? $inventories[$ctr]->getInventoryTypes()->getInventoryTypeId() : "", suffix: NULL, text: Constant::TEXT_NONE, value: DEFAULT_VALUE_BLANK);
                $output .= $option->getHtml();
                foreach ($inventoryTypes as $inventoryType) {
                    $option = new FormOption(debug: SessionUtility::getValue(SessionUtility::OBJECT_NAME_DEBUG), class: NULL, disabled: false, id: NULL, name: NULL, selectedValue: count($inventories) > 0 ? $inventories[$ctr]->getInventoryTypes()->getInventoryTypeId() : "", suffix: NULL, text: $inventoryType->getInventoryTypeName(), value: $inventoryType->getInventoryTypeId());
                    $output .= $option->getHtml();
                }
                $output .= "     </select>\n";
                $output .= "    </div>\n";
    ...
    foreach ($ary as $id) {
        if (Constant::MODE_SAVE_CREATE == $mode || Constant::MODE_SAVE_MODIFY == $mode) {
            ...
            if (Constant::MODE_SAVE_CREATE == $mode) {
                $in = new Inventories();
                $in->setInventoryComment($inventoryComment);
                $in->setInventoryCurrentAmount($currentAmount);
                $in->setInventoryWarningAmount($warningAmount);
                $in->setInventoryOrderAmount($orderAmount);
                $inventoryType = $entityManager->find(Constant::ENTITY_INVENTORY_TYPES, $inventoryTypeId);
                $in->setInventoryTypes($inventoryType);
                ...
                $resultList2 = $entityManager->getRepository(Constant::ENTITY_INVENTORIES)->getMaxId();
            ...
            if (Constant::MODE_SAVE_MODIFY == $mode) {
                $in = $entityManager->find(Constant::ENTITY_INVENTORIES, $tempInventoryId);
                $in->setInventoryComment($inventoryComment);
                $in->setInventoryCurrentAmount($currentAmount);
                $in->setInventoryWarningAmount($warningAmount);
                $in->setInventoryOrderAmount($orderAmount);
...
if (Constant::MODE_VIEW == $mode || Constant::MODE_DELETE == $mode || Constant::MODE_CONFIRM == $mode) {
    if (Constant::MODE_CONFIRM == $mode) {
        if (DEFAULT_VALUE_BLANK != $ids) {
            $in = $entityManager->find(Constant::ENTITY_INVENTORIES, (int) $ids);
    ...
    $output .= $hiddenSelectedRows->getHtml();
    $id = ("" == $ids) ? NULL : (int) $ids;
    $result = $entityManager->getRepository(Constant::ENTITY_INVENTORIES)->getTableOutput(inventoryId: $id, indexed: true);
    $resultHeaders = $entityManager->getRepository(Constant::ENTITY_INVENTORIES)->getTableOutput(inventoryId: $id, indexed: false);
    $colFormats = NULL;
