...
export const inputLocal = {
  ...
  validate : function() {
    const name = document.querySelectorAll("[id^='gameTypeName_']");
    if (name.length > 0) {
...
let documentReadyCallback = () => {
  ...
  inputLocal.validate();
  input.storePreviousValue({selectors: ["[id^='gameTypeName_']"]});
};
...
document.addEventListener("click", (event) => {
  ...
  if (event.target && event.target.id.includes("reset")) {
    input.restorePreviousValue({selectors: ["[id^='gameTypeName_']"]});
