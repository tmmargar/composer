...
if (Constant::MODE_CREATE == $mode || Constant::MODE_MODIFY == $mode) {
    $resultList = $entityManager->getRepository(Constant::ENTITY_GAME_TYPES)->getById(gameTypeId: (int) $ids);
    ...
    foreach ($ary as $id) {
        ...
        if (Constant::MODE_SAVE_CREATE == $mode) {
            ...
            $gt = $entityManager->find(Constant::ENTITY_GAME_TYPES, $gameTypeId);
            $gt->setGameTypeName($gameTypeName);
            $entityManager->persist($gt);
            try {
                $entityManager->flush();
...
if (Constant::MODE_VIEW == $mode || Constant::MODE_DELETE == $mode || Constant::MODE_CONFIRM == $mode) {
    if (Constant::MODE_CONFIRM == $mode) {
        if ($ids != DEFAULT_VALUE_BLANK) {
            $gt = $entityManager->find(Constant::ENTITY_GAME_TYPES, (int) $ids);
    ...
    $output .= $hiddenSelectedRows->getHtml();
    $id = ("" == $ids) ? NULL : (int) $ids;
    $result = $entityManager->getRepository(Constant::ENTITY_GAME_TYPES)->getTableOutput(gameTypeId: $id, indexed: true);
    $resultHeaders = $entityManager->getRepository(Constant::ENTITY_GAME_TYPES)->getTableOutput(gameTypeId: $id, indexed: false);
