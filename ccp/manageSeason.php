    ...
    foreach ($ary as $id) {
        ...
        if ($rowCount[0][0] > 0) {
            $output .=
                ...
                "  display.showErrors({errors: [ \"You must enter a start date (" . $ssd->format("m/d/Y") . ") and end date  (" . $sed->format("m/d/Y") . ") that do not overlap with an existing season\" ]});\n" .
                "</script>\n";
        } else {
