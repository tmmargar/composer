...
if (Constant::MODE_CREATE == $mode || Constant::MODE_MODIFY == $mode) {
    ...
    $resultList = $entityManager->getRepository(Constant::ENTITY_GROUP_PAYOUTS)->getById(groupId: Constant::MODE_MODIFY == $mode ? (int) $aryGroupPayoutIds[0] : 0, payoutId: Constant::MODE_MODIFY == $mode ? (int) $aryGroupPayoutIds[1] : 0);
    if (Constant::MODE_CREATE == $mode || (Constant::MODE_MODIFY == $mode && $ids != DEFAULT_VALUE_BLANK)) {
        ...
        foreach ($ary as $id) {
            ...
            $resultList2 = $entityManager->getRepository(Constant::ENTITY_GROUPS)->getById(groupId: NULL);
            if (count($resultList2) > 0) {
                ...
                foreach ($resultList2 as $group) {
                    $option = new FormOption(debug: SessionUtility::getValue(SessionUtility::OBJECT_NAME_DEBUG), class: NULL, disabled: false, id: NULL, name: NULL, selectedValue: count($resultList) > 0 ? $resultList[$ctr]->getGroups()->getGroupId() : "", suffix: NULL, text: $group->getGroupName(), value: $group->getGroupId());
                    $output .= $option->getHtml();
                }
                $output .= "  </select>\n";
                $output .= " </div>\n";
            ...
            $resultList2 = $entityManager->getRepository(Constant::ENTITY_PAYOUTS)->getById(payoutId: NULL);
            if (count($resultList2) > 0) {
                $output .= " <div class=\"responsive-cell responsive-cell-value\">";
                ...
                foreach ($resultList2 as $payout) {
                    ...
                    $option = new FormOption(debug: SessionUtility::getValue(SessionUtility::OBJECT_NAME_DEBUG), class: NULL, disabled: false, id: NULL, name: NULL, selectedValue: count($resultList) > 0 ? $resultList[$ctr]->getPayouts()->getPayoutId() : "", suffix: NULL, text: $payout->getPayoutName(), value: $payout->getPayoutId());
                    $output .= $option->getHtml();
                }
                $output .= "  </select>\n";
                $output .= " </div>\n";
    ...
    foreach ($ary as $id) {
        ...
        if (Constant::MODE_SAVE_CREATE == $mode) {
            ...
            $groupsFind = $entityManager->find(Constant::ENTITY_GROUPS, $groupId);
            $grp->setGroups($groupsFind);
            $payoutsFind = $entityManager->find(Constant::ENTITY_PAYOUTS, $payoutId);
            ...
            $groupsFind = $entityManager->find(Constant::ENTITY_GROUPS, $originalGroupId);
            $payoutsFind = $entityManager->find(Constant::ENTITY_PAYOUTS, $originalPayoutId);
            ...
            $groupsFind = $entityManager->find(Constant::ENTITY_GROUPS, $groupId);
            $groupPayoutsFind->setGroups($groupsFind);
            $payoutsFind = $entityManager->find(Constant::ENTITY_PAYOUTS, $payoutId);
            $groupPayoutsFind->setPayouts($payoutsFind);
            $entityManager->persist($groupPayoutsFind);
            try {
                $entityManager->flush();
            } catch (Exception $e) {
                $errors = $e->getMessage();
...
if (Constant::MODE_VIEW == $mode || Constant::MODE_DELETE == $mode || Constant::MODE_CONFIRM == $mode) {
    if (Constant::MODE_CONFIRM == $mode) {
        if (DEFAULT_VALUE_BLANK != $ids) {
            ...
            $groupsFind = $entityManager->find(Constant::ENTITY_GROUPS, (int) $aryGroupPayoutIds[0]);
            $payoutsFind = $entityManager->find(Constant::ENTITY_PAYOUTS, (int) $aryGroupPayoutIds[1]);
            $groupPayoutsFind = $entityManager->getRepository(Constant::ENTITY_GROUP_PAYOUTS)->findOneBy(array("groups" => $groupsFind, "payouts" => $payoutsFind));
    ...
    $result = $entityManager->getRepository(Constant::ENTITY_GROUP_PAYOUTS)->getTableOutput(groupId: "" == $aryGroupPayoutIds[0] ? NULL : (int) $aryGroupPayoutIds[0], payoutId: "" == $aryGroupPayoutIds[0] ? NULL : (int) $aryGroupPayoutIds[1], indexed: true);
    $resultHeaders = $entityManager->getRepository(Constant::ENTITY_GROUP_PAYOUTS)->getTableOutput(groupId: "" == $aryGroupPayoutIds[0] ? NULL : (int) $aryGroupPayoutIds[0], payoutId: "" == $aryGroupPayoutIds[0] ? NULL : (int) $aryGroupPayoutIds[1], indexed: false);
