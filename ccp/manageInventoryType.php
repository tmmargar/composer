...
if (Constant::MODE_CREATE == $mode || Constant::MODE_MODIFY == $mode) {
    ...
    $resultList = $entityManager->getRepository(Constant::ENTITY_INVENTORY_TYPES)->getById(inventoryTypeId: (int) $ids);
    ...
    foreach ($ary as $id) {
        if (Constant::MODE_SAVE_CREATE == $mode || Constant::MODE_SAVE_MODIFY == $mode) {
            ...
            if (Constant::MODE_SAVE_CREATE == $mode) {
                ...
                $it->setInventoryTypeComment($inventoryTypeComment);
                $it->setInventoryTypeMinAmount($inventoryTypeMinAmount);
                $it->setInventoryTypeMaxAmount($inventoryTypeMaxAmount);
                $it->setInventoryTypeName($inventoryTypeName);
                ...
                $resultList2 = $entityManager->getRepository(Constant::ENTITY_INVENTORY_TYPES)->getMaxId();
            ...
            if (Constant::MODE_SAVE_MODIFY == $mode) {
                $it = $entityManager->find(Constant::ENTITY_INVENTORY_TYPES, $tempInventoryTypeId);
                $it->setInventoryTypeComment($inventoryTypeComment);
                $it->setInventoryTypeMinAmount($inventoryTypeMinAmount);
                $it->setInventoryTypeMaxAmount($inventoryTypeMaxAmount);
                $it->setInventoryTypeName($inventoryTypeName);
...
if (Constant::MODE_VIEW == $mode || Constant::MODE_DELETE == $mode || Constant::MODE_CONFIRM == $mode) {
    if (Constant::MODE_CONFIRM == $mode) {
        if (DEFAULT_VALUE_BLANK != $ids) {
            $it = $entityManager->find(Constant::ENTITY_INVENTORY_TYPES, (int) $ids);
    ...
    $output .= $hiddenSelectedRows->getHtml();
    $id = ("" == $ids) ? NULL : (int) $ids;
    $result = $entityManager->getRepository(Constant::ENTITY_INVENTORY_TYPES)->getTableOutput(inventoryTypeId: $id, indexed: true);
    $resultHeaders = $entityManager->getRepository(Constant::ENTITY_INVENTORY_TYPES)->getTableOutput(inventoryTypeId: $id, indexed: false);
