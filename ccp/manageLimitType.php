...
define("LIMIT_TYPE_ID_FIELD_NAME", "limitTypeId");
define("LIMIT_TYPE_NAME_FIELD_NAME", "limitTypeName");
define("DEFAULT_VALUE_LIMIT_TYPE_ID", "-1");
$smarty->assign("title", "Manage Limit Type");
$smarty->assign("heading", "Manage Limit Type");
$smarty->assign("style", "<link href=\"css/manageLimitType.css\" rel=\"stylesheet\">");
$errors = NULL;
if (Constant::MODE_CREATE == $mode || Constant::MODE_MODIFY == $mode) {
    $resultList = $entityManager->getRepository(entityName: Constant::ENTITY_LIMIT_TYPES)->getById(limitTypeId: (int) $ids);
    ...
    $ary = explode(separator: Constant::DELIMITER_DEFAULT, string: $ids);
    foreach ($ary as $id) {
        $limitTypeId = (int) ((isset($_POST[HIDDEN_ROW_FIELD_NAME . "_" . $id])) ? $_POST[HIDDEN_ROW_FIELD_NAME . "_" . $id] : DEFAULT_VALUE_LIMIT_TYPE_ID);
        $limitTypeName = (isset($_POST[LIMIT_TYPE_NAME_FIELD_NAME . "_" . $id])) ? $_POST[LIMIT_TYPE_NAME_FIELD_NAME . "_" . $id] : "";
        if (Constant::MODE_SAVE_CREATE == $mode) {
            $gt = new LimitTypes();
            $gt->setLimitTypeName(limitTypeName: $limitTypeName);
            ...
            $gt = $entityManager->find(className: Constant::ENTITY_LIMIT_TYPES, id: $limitTypeId);
            $gt->setLimitTypeName(limitTypeName: $limitTypeName);
            $entityManager->persist(entity: $gt);
            try {
                $entityManager->flush();
...
if (Constant::MODE_VIEW == $mode || Constant::MODE_DELETE == $mode || Constant::MODE_CONFIRM == $mode) {
    if (Constant::MODE_CONFIRM == $mode) {
        if ($ids != DEFAULT_VALUE_BLANK) {
            $gt = $entityManager->find(className: Constant::ENTITY_LIMIT_TYPES, id: (int) $ids);
    ...
    $output .= $hiddenSelectedRows->getHtml();
    $id = ("" == $ids) ? NULL : (int) $ids;
    $result = $entityManager->getRepository(entityName: Constant::ENTITY_LIMIT_TYPES)->getTableOutput(limitTypeId: $id, indexed: true);
    $resultHeaders = $entityManager->getRepository(entityName: Constant::ENTITY_LIMIT_TYPES)->getTableOutput(limitTypeId: $id, indexed: false);
