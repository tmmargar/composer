...
if (Constant::MODE_CREATE == $mode || Constant::MODE_MODIFY == $mode) {
    $resultList = $entityManager->getRepository(Constant::ENTITY_SPECIAL_TYPES)->getById(specialTypeId: (int) $ids);
    ...
    foreach ($ary as $id) {
        ...
        if (Constant::MODE_SAVE_CREATE == $mode) {
            $st = new SpecialTypes();
            $st->setSpecialTypeDescription(addslashes($specialTypeDescription));
            $st->setSpecialTypeMultiplier($specialTypeMultiplier);
            ...
            $st = $entityManager->find(Constant::ENTITY_SPECIAL_TYPES, $specialTypeId);
            $st->setSpecialTypeDescription(addslashes($specialTypeDescription));
            $st->setSpecialTypeMultiplier($specialTypeMultiplier);
            $entityManager->persist($st);
            try {
...
if (Constant::MODE_VIEW == $mode || Constant::MODE_DELETE == $mode || Constant::MODE_CONFIRM == $mode) {
    if (Constant::MODE_CONFIRM == $mode) {
        if ($ids != DEFAULT_VALUE_BLANK) {
            $st = $entityManager->find(Constant::ENTITY_SPECIAL_TYPES, (int) $ids);
    ...
    $output .= $hiddenSelectedRows->getHtml();
    $id = ("" == $ids) ? NULL : (int) $ids;
    $result = $entityManager->getRepository(Constant::ENTITY_SPECIAL_TYPES)->getTableOutput(specialTypeId: $id, indexed: true);
    $resultHeaders = $entityManager->getRepository(Constant::ENTITY_SPECIAL_TYPES)->getTableOutput(specialTypeId: $id, indexed: false);
