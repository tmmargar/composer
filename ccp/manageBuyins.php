...
if (Constant::MODE_SAVE_CREATE == $mode || Constant::MODE_SAVE_MODIFY == $mode) {
  ...
  if (count($resultList) > 0) {
    $seasonId = (int) $resultList[0];
  }
  for ($index = 0; $index < count($aryPlayers); $index++) {
    if (" " != $aryFees[$index]) {
      $params = array((int) $aryFees[$index], (int) $seasonId, (int) $aryPlayers[$index], $tournamentId);
      $rowCount = $databaseResult->updateFees(params: $params);
      if (0 == $rowCount) {
        $params = array($seasonId, (int) $aryPlayers[$index]);
        $rowCount = $databaseResult->deleteFeeBySeasonAndPlayer(params: $params);
        $params = array($seasonId, (int) $aryPlayers[$index], $tournamentId, (int) $aryFees[$index]);
    ...
    $params = array($statusCode, $buyinPaid, $rebuyPaid, $addonPaid, $rebuyCount, $tournamentId, (int) $aryPlayers[$index]);
    ...
    while ($cnt < count($resultList)) {
      ...
      if (strpos($tournament->getDescription(), "Championship") === false) {
        ...
        if (0 < count($resultListNested)) {
          $total[$tournament->getId()] = (int) str_replace(search: ",", replace: "", subject: number_format((float) $resultListNested[0], 0));
        }
      }
