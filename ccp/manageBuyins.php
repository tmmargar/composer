...
if (Constant::MODE_SAVE_CREATE == $mode || Constant::MODE_SAVE_MODIFY == $mode) {
    ...
    $resultList = $entityManager->getRepository(Constant::ENTITY_SEASONS)->getForTournament(tournamentId: $tournamentId);
    if (count($resultList) > 0) {
        $seasonId = (int) $resultList[0]["season_id"];
    }
    for ($index = 0; $index < count($aryPlayers); $index++) {
        if (" " != $aryFees[$index]) {
            $fe = $entityManager->getRepository(Constant::ENTITY_FEES)->findOneBy(array("seasons" => (int) $seasonId, "players" => (int) $aryPlayers[$index], "tournaments" => $tournamentId));
            if (NULL !== $fe) {
                $fe->setFeeAmount((int) $aryFees[$index]);
                ...
                $rowCount = $entityManager->getRepository(Constant::ENTITY_FEES)->deleteForSeasonAndPlayer(seasonId: $seasonId, playerId: (int) $aryPlayers[$index]);
                $fe = new Fees();
                $fe->setFeeAmount((int) $aryFees[$index]);
                $player = $entityManager->find(Constant::ENTITY_PLAYERS, (int) $aryPlayers[$index]);
                $fe->setPlayers($player);
                $season = $entityManager->find(Constant::ENTITY_SEASONS, (int) $seasonId);
        ...
        $re = $entityManager->getRepository(Constant::ENTITY_RESULTS)->findOneBy(array("players" => (int) $aryPlayers[$index], "tournaments" => $tournamentId));
        $player = $entityManager->find(Constant::ENTITY_PLAYERS, (int) $aryPlayers[$index]);
        $re->setPlayers($player);
        $re->setResultPaidAddonFlag($addonPaid);
        $re->setResultPaidBuyinFlag($buyinPaid);
        $re->setResultPaidRebuyFlag($rebuyPaid);
        $re->setResultRebuyCount($rebuyCount);
        $statusCode = $entityManager->find(Constant::ENTITY_STATUS_CODES, $statusCode);
...
if ($mode == Constant::MODE_VIEW) {
    ...
    if (count($resultList) > 0) {
        ...
        while ($cnt < count($resultList)) {
            ...
            if (strpos($tournament->getDescription(), "Championship") === false) {
                ...
                $resultListNested = $entityManager->getRepository(Constant::ENTITY_TOURNAMENTS)->getPrizePool(startDate: new DateTime(SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_START_DATE)), endDate: new DateTime(SessionUtility::getValue(SessionUtility::OBJECT_NAME_END_DATE)));
        ...
        $hiddenSeasonFee = new FormControl(debug: SessionUtility::getValue(SessionUtility::OBJECT_NAME_DEBUG), accessKey: NULL, autoComplete: NULL, autoFocus: false, checked: NULL, class: NULL, cols: NULL, disabled: false, id: SEASON_FEE_FIELD_NAME, maxLength: NULL, name: SEASON_FEE_FIELD_NAME, onClick: NULL, placeholder: NULL, readOnly: false, required: NULL, rows: NULL, size: NULL, suffix: NULL, type: FormControl::TYPE_INPUT_HIDDEN, value: SessionUtility::getValue(SessionUtility::OBJECT_NAME_FEE), wrap: NULL);
    ...
    if ($tournamentId != DEFAULT_VALUE_TOURNAMENT_ID) {
        ...
        if ("" != $temp) {
            $output .= "<div class=\"responsive responsive--3cols responsive--collapse\">\n";
            $resultList = $entityManager->getRepository(Constant::ENTITY_FEES)->getAmountForTournament(tournamentId: $tournamentId);
            $feeCalc = " (";
            $feeTotal = 0;
            $feePlayerIds = "";
            ...
            $hiddenTotalSeasonFee = new FormControl(debug: SessionUtility::getValue(SessionUtility::OBJECT_NAME_DEBUG), accessKey: NULL, autoComplete: NULL, autoFocus: false, checked: NULL, class: NULL, cols: NULL, disabled: false, id: TOTAL_SEASON_FEE_FIELD_NAME, maxLength: NULL, name: TOTAL_SEASON_FEE_FIELD_NAME, onClick: NULL, placeholder: NULL, readOnly: false, required: NULL, rows: NULL, size: NULL, suffix: NULL, type: FormControl::TYPE_INPUT_HIDDEN, value: 0, wrap: NULL);
            ...
            $resultList = $entityManager->getRepository(Constant::ENTITY_PLAYERS)->getStatuses(tournamentId: $tournamentId, indexed: false);
            $countPaid = 0;
            if (count($resultList) > 0) {
                $ctr = 0;
