...
if (Constant::MODE_CREATE == $mode || Constant::MODE_MODIFY == $mode) {
    $notifications = $entityManager->getRepository(Constant::ENTITY_NOTIFICATIONS)->getById(notificationId: (int) $ids);
    ...
    foreach ($ary as $id) {
        ...
        if (Constant::MODE_SAVE_CREATE == $mode) {
            ...
            $nt->setNotificationDescription($notificationDescription);
            $nt->setNotificationStartDate(new DateTime(datetime: $notificationStartDate));
            $nt->setNotificationEndDate(new DateTime(datetime: $notificationEndDate));
            ...
            $notifications = $entityManager->find(Constant::ENTITY_NOTIFICATIONS, $notificationId);
            $notifications->setNotificationDescription($notificationDescription);
            $notifications->setNotificationStartDate(new DateTime(datetime: $notificationStartDate));
            $notifications->setNotificationEndDate(new DateTime(datetime: $notificationEndDate));
...
if (Constant::MODE_VIEW == $mode || Constant::MODE_DELETE == $mode || Constant::MODE_CONFIRM == $mode) {
    if (Constant::MODE_CONFIRM == $mode) {
        if ($ids != DEFAULT_VALUE_BLANK) {
            $notifications = $entityManager->find(Constant::ENTITY_NOTIFICATIONS, (int) $ids);
    ...
    $output .= $hiddenSelectedRows->getHtml();
    $id = ("" == $ids) ? NULL : (int) $ids;
    $result = $entityManager->getRepository(Constant::ENTITY_NOTIFICATIONS)->getTableOutput(notificationId: $id, indexed: true);
    $resultHeaders = $entityManager->getRepository(Constant::ENTITY_NOTIFICATIONS)->getTableOutput(notificationId: $id, indexed: false);
