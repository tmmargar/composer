...
if (Constant::MODE_CREATE == $mode || Constant::MODE_MODIFY == $mode) {
    $notifications = $entityManager->getRepository(entityName: Constant::ENTITY_NOTIFICATIONS)->getById(notificationId: (int) $ids);
    ...
    foreach ($ary as $id) {
        ...
        if (Constant::MODE_SAVE_CREATE == $mode) {
            ...
            $nt->setNotificationDescription(notificationDescription: $notificationDescription);
            $nt->setNotificationStartDate(notificationStartDate: new DateTime(datetime: $notificationStartDate));
            $nt->setNotificationEndDate(notificationEndDate: new DateTime(datetime: $notificationEndDate));
            ...
            $notifications = $entityManager->find(className: Constant::ENTITY_NOTIFICATIONS, id: $notificationId);
            $notifications->setNotificationDescription(notificationDescription: $notificationDescription);
            $notifications->setNotificationStartDate(notificationStartDate: new DateTime(datetime: $notificationStartDate));
            $notifications->setNotificationEndDate(notificationEndDate: new DateTime(datetime: $notificationEndDate));
            $entityManager->persist(entity: $notifications);
            try {
                $entityManager->flush();
            } catch (Exception $e) {
                $errors = $e->getMessage();
...
if (Constant::MODE_VIEW == $mode || Constant::MODE_DELETE == $mode || Constant::MODE_CONFIRM == $mode) {
    if (Constant::MODE_CONFIRM == $mode) {
        if ($ids != DEFAULT_VALUE_BLANK) {
            $notifications = $entityManager->find(className: Constant::ENTITY_NOTIFICATIONS, id: (int) $ids);
            $entityManager->remove(entity: $notifications);
            try {
                $entityManager->flush();
            } catch (Exception $e) {
                $errors = $e->getMessage();
    ...
    $output .= $hiddenSelectedRows->getHtml();
    $id = ("" == $ids) ? NULL : (int) $ids;
    $result = $entityManager->getRepository(entityName: Constant::ENTITY_NOTIFICATIONS)->getTableOutput(notificationId: $id, indexed: true);
    $resultHeaders = $entityManager->getRepository(entityName: Constant::ENTITY_NOTIFICATIONS)->getTableOutput(notificationId: $id, indexed: false);
    $colFormats = array(array(2, "datetime", 0), array(3, "datetime", 0));
