...
if (Constant::MODE_CREATE == $mode || Constant::MODE_MODIFY == $mode) {
    ...
    $numRows = array_key_exists(key: Constant::NAME_STATUS_REGISTERED, array: $valuesCount) ? $valuesCount[Constant::NAME_STATUS_REGISTERED] : 0;
    $output .=
        "<script type=\"module\">\n" .
        "  import { dataTable, display, input } from \"./scripts/import.js\";\n" .
        "  let aryMessages = [];\n";
    foreach ($ary as $index => $id) {
        $resultList = $entityManager->getRepository(entityName: Constant::ENTITY_PLAYERS)->getById(playerId: (int) $id);
        ...
        if ($aryStatus[$index] == ATTENDING_TEXT) {
            ...
            $players = $entityManager->find(className: Constant::ENTITY_PLAYERS, id: $playerId);
            $ta->setPlayers(players: $players);
            $tournaments = $entityManager->find(className: Constant::ENTITY_TOURNAMENTS, id: $tournamentId);
            $ta->setTournaments(tournaments: $tournaments);
            $entityManager->persist(entity: $ta);
            try {
                $entityManager->flush();
            } catch (Exception $e) {
                $errors = $e->getMessage();
            ...
            $players = $entityManager->find(className: Constant::ENTITY_PLAYERS, id: $playerId);
            $tournaments = $entityManager->find(className: Constant::ENTITY_TOURNAMENTS, id: $tournamentId);
            $ta = $entityManager->find(className: Constant::ENTITY_TOURNAMENT_ABSENCES, id: ["tournaments" => $tournaments, "players" => $players]);
            $entityManager->remove(entity: $ta);
            try {
                $entityManager->flush();
            } catch (Exception $e) {
                $errors = $e->getMessage();
...
if ($mode == Constant::MODE_VIEW) {
    ...
    $resultList = $entityManager->getRepository(entityName: Constant::ENTITY_TOURNAMENTS)->getAllMultiple(championship: true, tournamentDate: NULL, startTime: NULL, tournamentId: NULL, notEntered: true, ordered: false, mode: NULL, interval: NULL, limitCount: NULL);
    if (count(value: $resultList) > 0) {
    ...
    if ($tournamentId != DEFAULT_VALUE_TOURNAMENT_ID) {
        ...
        $buttonRegister = new FormControl(debug: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_DEBUG), accessKey: Constant::ACCESSKEY_ATTEND, autoComplete: NULL, autoFocus: false, checked: NULL, class: NULL, cols: NULL, disabled: false, id: Constant::TEXT_ATTEND, maxLength: NULL, name: Constant::TEXT_ATTEND, onClick: NULL, placeholder: NULL, readOnly: false, required: NULL, rows: NULL, size: NULL, suffix: NULL, type: FormControl::TYPE_INPUT_SUBMIT, value: Constant::TEXT_ATTEND_UNATTEND, wrap: NULL, noValidate: false);
        $output .= $buttonRegister->getHtml();
        $output .= " </div>\n";
        $result = $entityManager->getRepository(entityName: Constant::ENTITY_PLAYERS)->getChampionshipQualified(startDate: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_START_DATE), endDate: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_END_DATE), numTourneys: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_CHAMPIONSHIP_QUALIFY), indexed: true);
        $resultHeaders = $entityManager->getRepository(entityName: Constant::ENTITY_PLAYERS)->getChampionshipQualified(startDate: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_START_DATE), endDate: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_END_DATE), numTourneys: SessionUtility::getValue(name: SessionUtility::OBJECT_NAME_CHAMPIONSHIP_QUALIFY), indexed: false);
        $colFormats = NULL;
