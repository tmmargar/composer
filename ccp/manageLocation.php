...
if (Constant::MODE_CREATE == $mode || Constant::MODE_MODIFY == $mode) {
    $resultList = $entityManager->getRepository(Constant::ENTITY_LOCATIONS)->getById(locationId: (int) $ids);
    ...
    if (Constant::MODE_CREATE == $mode || (Constant::MODE_MODIFY == $mode && $ids != DEFAULT_VALUE_BLANK)) {
        ...
        foreach ($ary as $id) {
            ...
            $resultList2 = $entityManager->getRepository(Constant::ENTITY_PLAYERS)->getActives();
            if (count($resultList2) > 0) {
                ...
                foreach ($resultList2 as $player) {
                    $option = new FormOption(debug: SessionUtility::getValue(SessionUtility::OBJECT_NAME_DEBUG), class: NULL, disabled: false, id: NULL, name: NULL, selectedValue: count($resultList) > 0 ? $resultList[$ctr]->getPlayers()->getPlayerId() : "", suffix: NULL, text: $player->getPlayerName(), value: $player->getPlayerId());
                    $output .= $option->getHtml();
                }
                $output .= "  </select>\n";
                $output .= " </div>\n";
    ...
    foreach ($ary as $id) {
        ...
        if (Constant::MODE_SAVE_CREATE == $mode) {
            ...
            $lo = new Locations();
            $lo->setLocationAddress($address);
            $lo->setLocationCity($city);
            $lo->setLocationMap(NULL);
            $lo->setLocationMapLink($map);
            $lo->setLocationName($locationName);
            $lo->setLocationState($state);
            $lo->setLocationZipCode($zipCode);
            $player = $entityManager->find(Constant::ENTITY_PLAYERS, $playerId);
            $lo->setPlayers($player);
            ...
            $lo = $entityManager->find(Constant::ENTITY_LOCATIONS, $tempLocationId);
            $lo->setLocationAddress($address);
            $lo->setLocationCity($city);
            $lo->setLocationMap(NULL);
            $lo->setLocationMapLink($map);
...
if (Constant::MODE_VIEW == $mode || Constant::MODE_DELETE == $mode || Constant::MODE_CONFIRM == $mode) {
    if (Constant::MODE_CONFIRM == $mode) {
        if (DEFAULT_VALUE_BLANK != $ids) {
            ...
            $lo = $entityManager->find(Constant::ENTITY_LOCATIONS, (int) $ids);
    ...
    $output .= $hiddenSelectedRows->getHtml();
    $id = ("" == $ids) ? NULL : (int) $ids;
    $result = $entityManager->getRepository(Constant::ENTITY_LOCATIONS)->getTableOutput(locationId: $id, indexed: true);
